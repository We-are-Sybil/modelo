...
#import timeliney: *
#import "./typst/parts/title_page.typ": title_page
#import "./typst/parts/header_footer.typ": project_header, project_footer
#import "./typst/parts/signatures.typ": get_signature, create_signature

#let spanish-month(date) = {
  let months = (
    "Enero",
    "Febrero", 
    "Marzo",
...
// Configure text with Quicksand variable font
#set text(font: "Quicksand", weight: "regular", hyphenate: false)
#set par(justify: true)

// Configure headings
#show heading: set text(font: "Quicksand", weight: "bold")
#show strong: set text(weight: "bold")

#title_page(
  title: strong[Modelo de Empresa para Generación de Confianza y Autonomía],
  subtitle: "Estructura de Trabajo por Células con Métricas y Responsabilidad Distribuida",
  doc_number: "MOD-001-2025",
  date: [#date.display("[day]/[month]/[year]")],
  author: "Equipo de Desarrollo Organizacional",
  signatory_initials: "jps",
  header: project_header
)

#set page(header: project_header)
...
- *Bancolombia / Nequi (Colombia):* Documentado por MIT CISR como caso de transformación digital con nuevas formas de trabajo más ágiles y orientadas a equipos; Nequi se usó para aprender y luego escalar prácticas al banco matriz. Esto muestra que, en el contexto colombiano, la descentralización por equipos es viable si hay métricas, plataformas y gobierno claro. (cisr.mit.edu (@bancolombia_coordinating_multiple_digita), Bancolombia (@el_mit_toma_la_transformaci_n_de_bancolo))
- *Rappi (Colombia):* Su blog de ingeniería describe organización por *squads* y liderazgo distribuido en unidades de producto, con énfasis en autonomía y reglas de cultura de equipo. Es un referente local de células con responsabilidad sobre resultados. (Rappi Tech (@rappi_tech_engineering_rappi_com), Medium (@the_4_culture_rules_of_rappi_s_search_an))
- *Nubank (LatAm):* Publica prácticas internas de ingeniería y trabajo por equipos autónomos (*squads/tribes*), enfocadas en iteración rápida y mejora continua; útil como benchmark regional de trabajo por células orientadas a producto. (Building Nubank (@inside_nubank_s_engineering_discover_the))
- *Globant (LatAm):* Modelo de “Agile Pods” (células auto-organizadas con objetivos y marco operativo propio) usado en servicios y productos digitales; evidencia de que las células con autonomía y métricas claras escalan sin sobrecarga jerárquica. (Globant (@creating_competitive_advantage_in_the_di), Globant Investor Relations (@augmented_collaboration_globant_presents))
...
- *Ajuste cultural en Colombia:* pasar de control vertical a responsabilidad por resultados requiere transición. Empezar con lo legal-formal intacto reduce fricción y riesgos, mientras se mueven las decisiones al nivel del equipo. (Bancolombia/Nequi muestran esta senda gradual). (cisr.mit.edu (@bancolombia_coordinating_multiple_digita))
- *Menos sobrecarga gerencial, más velocidad:* células con métricas y límites de autonomía bien definidos disminuyen cuellos de botella y tiempos de entrega (casos Rappi/Nubank/Globant). (Rappi Tech (@rappi_tech_engineering_rappi_com), Building Nubank (@inside_nubank_s_engineering_discover_the), Globant (@creating_competitive_advantage_in_the_di))
